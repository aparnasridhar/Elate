...
public class MemoryProvider extends ContentProvider {
	...
	private static UriMatcher buildUriMatcher() {
		...
		final String authority = MemoryContract.CONTENT_AUTHORITY;
	...
	public boolean onCreate() {
        mOpenHelper = new MemoryDatabase(getContext());
		return true;
	}
	...
	public String getType(Uri uri) {
		...
		switch (match) {
			case ITEMS:
				return MemoryContract.Items.CONTENT_TYPE;
			case ITEMS__ID:
				return MemoryContract.Items.CONTENT_ITEM_TYPE;
			default:
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch (match) {
			case ITEMS: {
				...
				return MemoryContract.Items.buildItemUri(_id);
			}
			default: {
				throw new UnsupportedOperationException("Unknown uri: " + uri);
			}
	...
	private SelectionBuilder buildSelection(Uri uri, int match, SelectionBuilder builder) {
		...
		switch (match) {
			...
			case ITEMS__ID: {
				...
				return builder.table(Tables.ITEMS).where(MemoryContract.Items._ID + "=?", _id);
			}
			default: {
				throw new UnsupportedOperationException("Unknown uri: " + uri);
			}
